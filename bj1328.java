...
public class bj1328 {
	...
	public static void main(String[] args) throws IOException {
		...
		int l = Integer.parseInt(st.nextToken()); // 왼쪽에서 보았을 떄 빌딩의 개수 
		...
		for (int i = 2;i< n+1; i++) {
		       ...
		       for (int j = 1; j< l+1; j++) {
		           for (int k = 1; k< r+1; k++) {
// 왼쪽의 개수가 달라졌을 떄(왼쪽에 n 번째 빌딩(크기가 1인 빌딩)을 세우는 경우), 오른쪽 건물의 개수가 달라졌을 때(오른쪽에 n 번째 빌딩을 세우는 경우), 
// 왼쪽과 오른쪽 건물의 개수는 변화 없고 건물의 개수가 변했을 경우(양 끝을 제외하고 나머지 장소에 건물을 세우는 경우)
		        	   //N = 4이면 N = 3일 때 크기 1인 빌딩을 추가해주는 방식으로 N = 4일 떄의 경우의 수를 구하자( DP 방식이다)
